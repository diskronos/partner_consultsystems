...
<script>
	...
	function validLogin(){
		$.post("/<?php echo URL::url_to_route('site-registration:ajax_signup_check_login')?>", { v: $("#user_login").val() },
		function(data){
	...
	function validPass(){
		$.post("/<?php echo URL::url_to_route('site-registration:ajax_signup_check_pass')?>", { v: $("#user_pass").val() },
		function(data){
	...
	function validPassAgain(){
		$.post("/<?php echo URL::url_to_route('site-registration:ajax_signup_check_pass_confirm')?>", { v1: $("#user_pass").val(), v2: $("#user_pass_again").val() },
		function(data){
	...
	function validMail(){
		$.post("/<?php echo URL::url_to_route('site-registration:ajax_signup_check_email')?>", { v: $("#user_mail").val() },
		function(data){
	...
	function validName(){
		$.post("/<?php echo URL::url_to_route('site-registration:ajax_signup_check_name')?>", { v: $("#user_name").val() },
		function(data){
	...
	function signUpSubmit(){
		$("#loading").show();
		$.post("/<?php echo URL::url_to_route('site-registration:ajax_proceed_registration')?>", $('form').serialize(),
			function(data){
				$("#loading").hide();
...
<div id="signupblock">
<br>
<h1>Регистрация <?php if (isset($referrer_id)) echo 'клиента '; ?>в системе WebConsult</h1>
<form action="/signup/" method="post">
	<input type="hidden" name="subm" value="1">
	<div id="user_login_block" class="form_elem">
		<div class="form_caption_block">логин:</div>
		<div class="form_input_block"><input class="forminput" type="text" id="user_login" name="name" value=""></div>
	...
	<div id="user_name_block" class="form_elem">
		...
		<div class="form_input_block"><input class="forminput" type="text" id="user_name" name="fullname" value=""></div>
		<div class="form_hint_block">Укажите ваше имя,<br>чтобы мы знали как к Вам обращаться.</div>
		<div class="form_error_block"></div>
	</div> 
	<div class="clear"></div>
	<input type="hidden" name="referrer_id" value="<?php if (isset($referrer_id)) echo $referrer_id?>">
	<in>
	<br>
	<div class="hr"></div>
	<br>
...
<br><br><br><br>
	...
	<span class="big3">Вы успешно <?php if (isset($referrer_id)) echo 'зарегистрировали клиента'; else echo 'зарегистрировались'; ?> в системе WebConsult. Информация о аккаунте отправлена на указанный почтовый ящик.</span>
	<br><br>
	<span class="big3"><strong>Нажмите "Далее" для продолжения.</strong></span>
	<br><br>
	<a href="/<?php echo URL::url_to_route($ref_route)?>"><img src="/images/next.png" border="0"></a>
	</div>
</div>
