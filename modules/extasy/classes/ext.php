...
class Ext
{
	...
	{
		$link = html::link_to_route($route, $title);
	...
	static public function submit($name, $value, $confirm = NULL)
	{
		$attributes = array(
			'class' => 'SaveButton',
			'style' => 'float: none;'
		...
		return Form::submit($name, $value, $attributes);
	}
	...
	static public function menu_row($id, $name, $route = NULL, $parent = -1)
	{
		if ( ! is_null($route) )
		{
			$url = url::base().url::url_to_route($route);
		}
		else
		{
			$url = NULL;
	...
	static public function crumbs(array $crumbs = array())
	{
		...
		{
			if($link = html::link_to_route($crumb['route'], $crumb['title']))
			{
				$links[] = $link;
				$crumbs_out[] = $crumb;
			}
